...
```
newsfetch/            Python 套件
  config.py           基礎詞庫：SEED_TERMS、TOPICS、SUBCATEGORIES
  rules.py            實際生效規則＝基礎詞庫＋已採用－已停用（keyword_rules 表）
  keyword_learning.py 關鍵字學習：從手動新增與審核結果產生新增／停用建議
...
| Workflow | 觸發 | 內容 |
|---|---|---|
| `daily-crawl.yml` | 每天台灣時間 08:00，也可手動執行（可只跑指定關鍵字） | 爬蟲 → 待審核佇列 → 前端 JSON → 執行報告 |
...
每次輸出資料時，系統會從審核紀錄統計出關鍵字建議（不使用 AI），顯示在「待審核」畫面底部的
「關鍵字學習」區塊，也列在每日執行報告中。建議只在你按下採用後才生效，並記錄當時的佐證。

| 訊號 | 判斷 | 建議 |
|---|---|---|
| 手動新增的文章（爬蟲漏抓），標題／標籤不含任何具體搜尋詞 | 關鍵字缺口 | 新增**搜尋關鍵字** |
| 手動新增的文章，標題／標籤已含具體搜尋詞 | 來源抓取問題（擴充關鍵字無法解決） | 只統計，不建議 |
| 已收錄文章中，現行規則判斷不出、但人工歸入某議題 | 議題規則缺口 | 新增**議題規則關鍵字** |
| 規則關鍵字命中後常被移除／整篇刪除；搜尋關鍵字找到的文章多半被刪除 | 誤觸 | **停用**該關鍵字 |

- 候選詞彙取自報社編輯下的 meta-keywords 與標題中的英文縮寫；排除籠統標籤、子分類關鍵字、
  金融機構名稱，以及已屬於其他議題的規則詞。
- 「金管會」「彭金隆」這類廣泛搜尋詞（`config.BROAD_SEED_GROUPS`）不算作「已涵蓋」，
  因為搜尋結果每天只取前幾十筆。
- 採用後隔天的爬蟲與分類即使用新規則，同時被新規則涵蓋的其他建議會自動消失；「忽略」的建議不再出現。
- 門檻（最少佐證篇數、準確率、誤觸率）在 `keyword_learning.py` 開頭調整。
- 基礎詞庫仍在 `config.py`；學到的關鍵字存在資料庫 `keyword_rules` 表，想永久寫回程式碼時可以一起討論整理。
...
- 資料表另外新增：`published_time`（清單顯示時間）、`pending_review.delete_reason`（刪除原因）、
  ...
  `keyword_rules`（關鍵字學習結果）、`pending_review.search_term`（由哪個搜尋詞找到）、
  `pending_review_suggestions.matched_terms`（建議命中哪些規則詞，用於計算誤觸率）。
